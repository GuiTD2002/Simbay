...
traj1 = plan_joints_trajectory(FRANKA_HOME_QPOS, pre_grasp_q7, max_velocity=0.25, dt=dt)
arm_traj1 = [qpos[:7] for qpos in traj1] # Extract just the 7 arm joints
real_robot.move_trajectory(arm_traj1, dt) # Send the whole smooth map!
...
traj3 = plan_joints_trajectory(pre_grasp_q7, grasp_q7, max_velocity=0.25, dt=dt)
arm_traj3 = [qpos[:7] for qpos in traj3]
real_robot.move_trajectory(arm_traj3, dt)
...
traj5 = plan_joints_trajectory(grasp_q7, pre_grasp_q7, max_velocity=0.25, dt=dt)
arm_traj5 = [qpos[:7] for qpos in traj5]
real_robot.move_trajectory(arm_traj5, dt)
